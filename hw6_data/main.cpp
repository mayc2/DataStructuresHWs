...
int main(int argc, char* argv[]) {
  ...
  vector<Vertex*> moves = board.getMoves();
  for (int i = 0; i < moves.size(); i++){
	  cout << moves[i]->direction << " "<< moves[i]->robot << endl;

  }
  vector<Position> positions;
  for (int i = 0; i < board.numRobots(); i++){
	  positions.push_back(board.getRobotPosition(i));
  }
  //cout << board.getGoalRobot(0) << board.getGoalRobot(1) << board.getGoalRobot(2) << endl;
  //BFS ON MOVES
  while (moves.size() != 0){
	  board.reset(positions);
	  Vertex *move = moves.front();
	  moves.erase(moves.begin());
	  if (board.makeMove(move)) break;
	  vector<Vertex*> temp_moves = board.getMoves();
	  for (int i = 0; i < temp_moves.size(); i++) {
		  temp_moves[i]->path = move; 
		  moves.push_back(temp_moves[i]);
	  }
