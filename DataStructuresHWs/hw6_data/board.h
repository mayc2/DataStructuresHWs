...
class Vertex{
public:
	Vertex(const int & r, const string & d): robot(r), direction(d), path(NULL), level(0){}
...
public:
  ...
  // PRINT
  void print();
  void reset(vector<Position> &pos);

  //get total possible moves
  vector<Vertex*> getMoves();
  bool canMove(int i, string &direction);
  bool makeMove(Vertex * move);
private:
